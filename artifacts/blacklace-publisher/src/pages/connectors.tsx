...
import { Plug, Activity, Key, Clock, RefreshCw, ExternalLink } from "lucide-react";
              ...
              <Card key={provider.id} className="border-border bg-card">
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between gap-3">
                    <div><CardTitle className="text-lg">{provider.label}</CardTitle><p className="font-mono text-[10px] uppercase text-muted-foreground mt-1">{provider.capability}</p></div>
                    <Badge variant={provider.status === "connected" ? "default" : "outline"}>{providerStatusLabel(provider)}</Badge>
                  </div>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  {provider.status === "connected" ? (
                    ...
                    <Button size="sm" className="w-full" onClick={() => void connect(provider)} disabled={!catalog?.configured || pendingProvider === provider.id}>
                      <ExternalLink className="w-4 h-4 mr-2" />{pendingProvider === provider.id ? "Ouverture…" : "Connecter"}
                    </Button>
                  )}
                </CardContent>
              </Card>
...
export default function Connectors() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-hidden">
        ...
        <div className="space-y-8">
          {groupedConnectors.map((family) => (
            <section key={family.id} className="space-y-3 min-w-0">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><h2 className="text-xl font-serif font-semibold text-foreground break-words">{family.label}</h2><p className="text-sm text-muted-foreground break-words">{family.description}</p></div><Badge variant="outline" className="w-fit font-mono text-[10px] uppercase">{family.connectors.length} liaison{family.connectors.length > 1 ? "s" : ""}</Badge></div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {family.connectors.map((connector) => (
                  <Card key={connector.name} className="bg-card border-border hover:border-primary/30 transition-colors flex flex-col relative overflow-hidden group min-w-0">
                    <div className={cn("absolute top-0 left-0 w-full h-1", connector.status === "connected" ? "bg-green-500/50" : connector.status === "error" ? "bg-destructive/50" : connector.status === "mock" ? "bg-amber-500/50" : "bg-muted")}></div>
                    <CardHeader className="pb-3"><div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-start"><div className="flex items-start gap-3 min-w-0"><div className="p-2 bg-secondary rounded border border-border shrink-0"><Plug className="w-5 h-5 text-primary" /></div><div className="min-w-0"><CardTitle className="text-xl font-serif break-words">{connector.displayName}</CardTitle><div className="flex items-center gap-1.5 mt-1"><Activity className={cn("w-3 h-3 shrink-0", STATUS_COLORS[connector.status])} /><span className="font-mono text-[10px] uppercase text-muted-foreground break-words">{connector.status}</span></div></div></div><RealMockIndicator result={lastResults[connector.name]} /></div></CardHeader>
                    <CardContent className="flex-1 flex flex-col justify-between space-y-4 min-w-0">
                      <p className="text-sm text-muted-foreground min-h-[40px] break-words">{connector.description || "Liaison de donnees non documentee."}</p>
                      {lastResults[connector.name] && <div className="rounded-md border border-border bg-secondary/20 p-3 space-y-1 min-w-0 overflow-hidden"><p className="text-xs text-foreground font-mono break-words">{lastResults[connector.name].message}</p>{lastResults[connector.name].source && <p className="text-[11px] font-mono text-muted-foreground break-words">Source : {lastResults[connector.name].title ?? lastResults[connector.name].source}{typeof lastResults[connector.name].charCount === "number" ? ` - ${lastResults[connector.name].charCount} caracteres` : ""}{typeof lastResults[connector.name].sectionCount === "number" ? ` - ${lastResults[connector.name].sectionCount} section(s)` : ""}</p>}{lastResults[connector.name].error && <p className="text-[11px] font-mono text-destructive break-words">{lastResults[connector.name].error}</p>}</div>}
                      <div className="space-y-3 pt-4 border-t border-border/50"><div className="flex items-center gap-1.5 text-muted-foreground font-mono text-[10px] uppercase tracking-wider"><Key className="w-3 h-3" /> Identifiants requis</div><div className="flex flex-wrap gap-2">{connector.requiredVars.map((variable) => <Badge key={variable} variant="outline" className="font-mono text-[10px] bg-secondary/30 max-w-full break-words">{variable}</Badge>)}</div></div>
                      <div className="pt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="text-[10px] font-mono text-muted-foreground flex items-center gap-1 min-w-0 break-words"><Clock className="w-3 h-3 shrink-0" />{connector.lastTestedAt ? format(parseISO(connector.lastTestedAt), "dd MMM yyyy HH:mm", { locale: fr }) : "Jamais teste"}</div><Button variant="outline" size="sm" className="w-full sm:w-auto font-mono text-xs border-primary/50 text-primary hover:bg-primary/20" onClick={() => testConnector.mutate({ name: connector.name })} disabled={testConnector.isPending}>{testConnector.isPending ? "Ping..." : "Ping"}</Button></div>
                    </CardContent>
                  </Card>
                ))}
              </div>
      ...
      ) : <div className="p-12 text-center border border-dashed border-border rounded-lg bg-card/50"><Plug className="w-8 h-8 text-muted-foreground mx-auto mb-4" /><h3 className="text-lg font-serif mb-2">Aucune liaison</h3><p className="text-muted-foreground font-mono text-sm">Le systeme est isole.</p></div>}
    </div>
  );
}
