...
const parcels: MissionParcel[] = ["Yael Bali", "Blacklace", "Benoît / Personnel", "Nouveau client"];
...
export function MissionPanel() {
  ...
  return (
    <section className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(320px,420px)] gap-6">
        ...
        <CardContent className="space-y-5 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={parcel} onValueChange={(value) => setParcel(value as MissionParcel)}>
      ...
      <Card className="bg-card border-border shadow-md">
        <CardHeader className="border-b border-border/50 pb-4">
          <CardTitle className="font-serif">Missions envoyées à Octopus</CardTitle>
              ...
              {missions.map((mission) => (
                <article key={mission.id} className="p-4 border border-border rounded-md bg-secondary/20 space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="font-serif font-semibold text-foreground">{mission.parcel}</h3>
                  ...
                  {mission.octopusResponse ? (
                    <div className="space-y-3 rounded-md border border-border bg-background/40 p-3">
                      ...
                      <p className="text-xs font-mono text-primary">Octopus a reçu la mission</p>
                      <div className="space-y-2">
                        <p className="text-[11px] font-mono uppercase text-muted-foreground">Graines proposées</p>
                        <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        ...
                        {mission.octopusResponse.nextActions.length > 0 ? (
                          ...
                          <p className="text-xs text-muted-foreground">Aucune action suivante proposée pour l'instant.</p>
                        )}
                      </div>
                      <div className="space-y-1">
