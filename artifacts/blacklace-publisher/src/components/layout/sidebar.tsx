...
const NAV_GROUPS = [
  {
    label: "Observer et produire",
    items: [
      { href: "/", label: "Vue d’ensemble", icon: LayoutDashboard },
      { href: "/greenhouse", label: "Récoltes", icon: PackageCheck },
      { href: "/observatory", label: "Observatoire", icon: Telescope },
      { href: "/radar", label: "Radar", icon: Radar },
      { href: "/memory", label: "Knowledge Packages", icon: Database },
      { href: "/autonomy", label: "Routine Gérard", icon: Bot },
    ],
  },
  {
    label: "Configurer",
    items: [
      { href: "/client", label: "Livres et applications", icon: Building2 },
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3 text-primary">
            ...
            <div>
              ...
              <span className="text-xs font-mono text-muted-foreground uppercase tracking-wider">Connaissances & récoltes</span>
            </div>
          </div>
          <button
      ...
      <nav className="flex-1 overflow-y-auto p-4">
        {NAV_GROUPS.map((group) => (
          <section key={group.label} className="mb-6 last:mb-0">
            <p className="mb-2 px-3 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground/70">{group.label}</p>
                  ...
                  <Link key={item.href} href={item.href}>
                    <div
                      className={cn(
                        ...
                        isActive ? "border-primary/20 bg-primary/10 text-primary" : "border-transparent text-muted-foreground hover:bg-secondary hover:text-foreground",
                      )}
                      onClick={onClose}
                    >
