...
export interface KnowledgeActivationRule {
  missionTypes: string[];
...
export interface KnowledgeItem {
  ...
  activationRules: KnowledgeActivationRule;
  relatedSeeds: string[];
  testedOnParcels: string[];
  observations: number;
...
export interface DigestObservation {
  ...
  activationRules?: Partial<KnowledgeActivationRule>;
  relatedSeeds?: string[];
}
...
function maturityFor(observations: number, successfulUses: number): KnowledgeMaturity {
  if (successfulUses >= 5) return "reference";
  if (successfulUses >= 1) return "proven";
  if (observations >= 2) return "validated";
  return "observed";
}
...
export async function digestObservation(input: DigestObservation): Promise<{ action: "created" | "enriched"; item: KnowledgeItem }> {
  ...
  if (!existing) {
    const item: KnowledgeItem = {
      ...
      expertises: unique(input.expertises),
      ...
      relatedSeeds: unique(input.relatedSeeds),
      testedOnParcels: [],
      observations: 1,
      successfulUses: 0,
      createdAt: now,
  ...
  const item: KnowledgeItem = {
    ...
    categories: unique([...previous.categories, ...(input.categories || [])]),
    expertises: unique([...previous.expertises, ...(input.expertises || [])]),
    tags: unique([...previous.tags, ...(input.tags || [])]),
    ...
    observations,
    maturity: maturityFor(observations, previous.successfulUses),
    updatedAt: now,
  };
  await writeGlobalState(NAMESPACE, key, item);
...
export async function recordKnowledgeUse(id: string, parcelId: string, successful: boolean): Promise<KnowledgeItem | null> {
  ...
  const item: KnowledgeItem = {
    ...previous,
    testedOnParcels: unique([...previous.testedOnParcels, parcelId]),
    successfulUses,
    maturity: maturityFor(previous.observations, successfulUses),
...
export async function listKnowledge(): Promise<KnowledgeItem[]> {
  const records = await listGlobalState<KnowledgeItem>(NAMESPACE);
  return records.map((record) => record.value).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
}

export async function selectKnowledgeForMission(input: {
  missionType?: string;
  artifactType?: string;
  audienceTags?: string[];
  expertises?: string[];
  ...
  const audiences = unique(input.audienceTags);
  return items.map((item) => {
    ...
    score += audiences.filter((value) => item.activationRules.audienceTags.includes(value) || item.tags.includes(value)).length * 0.15;
    score += ({ observed: 0, validated: 0.15, proven: 0.35, reference: 0.5 })[item.maturity];
    return { item, score };
  }).filter(({ score }) => score > 0.7).sort((a, b) => b.score - a.score).slice(0, Math.max(1, Math.min(input.limit || 8, 20))).map(({ item }) => item);
}
